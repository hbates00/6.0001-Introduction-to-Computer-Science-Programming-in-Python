# -*- coding: utf-8 -*-
# Problem Set 5: Simulating robots
# Name:
...
import ps5_visualize
import pylab

# For python 2.7:
from ps5_verify_movement27 import test_robot_movement
...
class RectangularRoom(object):
    ...
    def __init__(self, width, height, dirt_amount):
        ...
        for x in xrange(height):
            ...
            for y in xrange(width):
                row_list.append(dirt_amount)
            self.dirt_list.append(row_list)
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        y = pos.get_x()
        x = pos.get_y()
        self.dirt_list[int(x)][int(y)] -= capacity        
        
        if self.dirt_list[int(x)][int(y)] < 0:
            self.dirt_list[int(x)][int(y)] = 0
        else:
            pass
...
testpos = Position(2,4)

test = RectangularRoom(4, 6, 10)
test.clean_tile_at_position(testpos, 4)
print test.dirt_list
...
# === Problem 4
class SickRobot(Robot):
    """
    A SickRobot is a robot that will dirty the tile it moves to and
    ...
    @staticmethod
    def set_sick_probability(prob):
        """
        Sets the probability of getting sick equal to PROB.
    ...
    def gets_sick(self):
        """
        Answers the question: Does this SickRobot get sick at this timestep?
        A SickRobot get sick with probability p.

        returns: True if the SickRobot gets sick, False otherwise.
        """

        return random.random() < SickRobot.p
    
    def update_position_and_clean(self):
        """
        Simulate the passage of a single time-step.

        Check if the robot gets sick. If the robot gets sick,
        dirty its current tile and change its direction randomly.
        If the robot does not get sick, the robot
        should behave like StandardRobot at this time-step
        (checking if it can move to a new position, move there if it can,
        pick a new direction and stay stationary if it can't)
        """
        # implement this method
        raise NotImplementedError
            
    
#test_robot_movement(SickRobot, EmptyRoom)
...
#avg = run_simulation(3, 1.0, 1, 20, 20, 3, 1.0, 50, StandardRobot)
#print avg
...
def show_plot_compare_strategies(title, x_label, y_label):
    ...
    for num_robots in num_robot_range:
        ...
        times1.append(runSimulation(num_robots, 1.0, 1, 20, 20, 3, 0.8, 20, StandardRobot))
        times2.append(runSimulation(num_robots, 1.0, 1, 20, 20, 3, 0.8, 20, SickRobot))
    pylab.plot(num_robot_range, times1)
    pylab.plot(num_robot_range, times2)
    pylab.title(title)
    pylab.legend(('StandardRobot', 'SickRobot'))
...
def show_plot_room_shape(title, x_label, y_label):
    ...
    for width in [10, 20, 25, 50]:
        ...
        times1.append(runSimulation(2, 1.0, 1, width, height, 3, 0.8, 200, StandardRobot))
        times2.append(runSimulation(2, 1.0, 1, width, height, 3, 0.8, 200, SickRobot))
